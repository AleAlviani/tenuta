...
if ( $_POST ):
    ...
    $user_AppointmentDate = filter_var( trim($_POST["appointment_date"]), FILTER_SANITIZE_STRING );
    ...
    $mail->CharSet = "UTF-8";    
    $mail->Subject = sprintf( $appointment["new_appointment"], $user_Name );

    $appointmentRes = "";
    $appointmentRes .= parseLine( $appointment["new_appointment"], $user_Name );
    $appointmentRes .= parseLine( $appointment["department"], $department );
    $appointmentRes .= parseLine( $appointment["appointment_date"], $user_AppointmentDate );
    $appointmentRes .= parseLine( $appointment["client_section"] );
    $appointmentRes .= parseLine( $appointment["birthdate"], $birthdate );
    $appointmentRes .= parseLine( $appointment["sex"], $sex );
    $appointmentRes .= parseLine( $appointment["email"], $user_Email );
    $appointmentRes .= parseLine( $appointment["phone"], $user_Phone );
    $appointmentRes .= parseLine( $appointment["message"], $user_Message );
    $appointmentRes .= parseLine( $appointment["sent_from"] );

    $mail->Body    = ($formText)? $appointmentRes : $user_Message;
